...
- Doesn't use session but RESTful API to authenticate. In Angular, the client and server are decoupled. A session would have to be stored on the server but since the server doesn't care about the client then session is not a solution.
- Once authenticated, the server will send the client a JSON token, an encoded (not encrypted) string with metadata. The string is generated by the server with a certain algorithm and private that only the server knows. Stored token is sent to authorize in subsequent requests.
- We can't alter or generate a token from the client, or else it will not fit the algorithm and private key used in the server. The server can determine the token is invalid and therefore block access.

### Preparing the backend
- Lock access to database resources so only authorized users have read and write permissions.
- Enable authentication on the server side. (e.g., Firebase Auth REST API)

### Preparing the signup request
Interface matches the response provided by the authentication API.
```typescript
